...
static void check(bool ok, const char* tag, const char* msg) {
    if (ok) std::fprintf(stdout, "%s (%s): PASS\n", tag, msg);
    else   { std::fprintf(stderr, "%s FAIL: %s\n", tag, msg); ++g_fails; }
}

// helper: HTTP GET returning full response
...
// helper: HTTP POST returning status code
static int http_post(const char* path, const char* body, int port) {
    ...
    while((n=::recv(fd,buf,sizeof(buf),0))>0) resp.append(buf,(size_t)n);
    ::close(fd);
    int code=0; sscanf(resp.c_str(),"HTTP/1.%*d %d",&code); return code;
}
...
// =============================================================================
// GUI endpoint gate tests (GE1–GE10)
// =============================================================================

static void test_gui_endpoints(int port) {
    // GE1: GET /sim-config initially returns {}
    std::string r = http_get("/sim-config", port);
    check(r.find("{}") != std::string::npos, "GE1", "GET /sim-config returns {}");

    // GE2: POST /sim-config stores body
    int code = http_post("/sim-config", "{\"cfl\":0.3}", port);
    check(code == 200, "GE2", "POST /sim-config 200");

    // GE3: GET /sim-config returns stored body
    r = http_get("/sim-config", port);
    check(r.find("cfl") != std::string::npos, "GE3", "GET /sim-config returns stored body");

    // GE4: POST /steer pause
    code = http_post("/steer", "{\"cmd\":\"pause\"}", port);
    check(code == 200, "GE4", "POST /steer pause 200");

    // GE5: POST /steer resume
    code = http_post("/steer", "{\"cmd\":\"resume\"}", port);
    check(code == 200, "GE5", "POST /steer resume 200");

    // GE6: GET /geometry has count field
    r = http_get("/geometry", port);
    check(r.find("count") != std::string::npos, "GE6", "GET /geometry has count field");

    // GE7: POST /primitives sphere 200
    code = http_post("/primitives",
        "{\"type\":\"sphere\",\"cx\":0.5,\"cy\":0.5,\"cz\":0.5,\"r\":0.2}", port);
    check(code == 200, "GE7", "POST /primitives sphere 200");
    ...
    code = http_post("/primitives",
        "{\"type\":\"box\",\"x0\":0.1,\"y0\":0.1,\"z0\":0.1,\"x1\":0.9,\"y1\":0.9,\"z1\":0.9}", port);
    check(code == 200, "GE9", "POST /primitives box 200");

    // GE10: POST /primitives cylinder 200
    code = http_post("/primitives",
        "{\"type\":\"cylinder\",\"cx\":0.5,\"cy\":0.5,\"cz0\":0.1,\"cz1\":0.9,\"r\":0.2}", port);
    check(code == 200, "GE10", "POST /primitives cylinder 200");
}
