# to_develop — Branch Configuration

This document specifies the Claude Code configuration for the `to_develop` branch.
...
Grants blanket tool access with three hard safety rails: no force-push, no
filesystem wipe, no hard-reset past HEAD.

---

## `CLAUDE.md` — development branch
...
- Ensure all gates on `to_refactor` pass on this branch: `cmake --build build -t ba`
...
1. **Research** — web-search latest literature; spawn Explore sub-agent to map
   affected files
2. **Plan** — write implementation plan to `docs/superpowers/plans/<date>-D<n>.md`
...
## Comparison with `to_refactor`

| Aspect | `to_refactor` | `to_develop` |
|---|---|---|
| **Goal** | Clean architecture | State-of-the-art physics + GPU performance |
| **Autonomy** | Confirm before commits | Commit on every green gate; pause before push only |
| **CPU fallback** | Allowed (AMR path) | Eliminated in D1 |
| **MPI staging** | CPU D2H→MPI→H2D | NCCL P2P after D2 |
| **Scheme order** | WENO5-Z (5th order) | TENO7-A (7th order) after D3 |
| **Implicit solver** | IMEX stub | Full GPU-resident GMRES after D4 |
| **New physics** | None (refactor only) | Reactions (D5), radiation (D6), WMLES (D7) |
| **Profiling** | Optional | Mandatory per new kernel; logged to `docs/perf/` |
| **Sub-agents** | Sparingly | Freely used for research, review, benchmarks |

## Setup instructions

```bash
# Create the branch from to_refactor
git checkout to_refactor
git checkout -b to_develop

# Copy this configuration into place
mkdir -p .claude
cp to_develop.md CLAUDE.md
...
# Verify baseline before starting D0
cmake --build build -t ba
```
