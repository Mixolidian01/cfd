...
| # | Status | Item | Why it matters |
|---|--------|------|----------------|
| P2.5 | 🔲 | On-device CFL reduction (`atomicMin_double` kernel) | Eliminates ~40 MB D→H transfer per step; prerequisite for P2.6 |
| P2.6 | 🔲 | CUDA Graph capture of full RK3 loop (requires P2.5) | Eliminates ~30 s of kernel-launch overhead at 10⁶ steps |
| T5   | 🔲 | GPU gate test (currently skipped — needs `nvcc`) | Validates GPU path end-to-end |
