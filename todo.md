...
| # | Status | Item | Reference | Effort |
|---|--------|------|-----------|--------|
| P14.1 | ⚠️ | **ACDI compressible multiphase** — Phase 1+2 (infrastructure + scalar advection + compression): `phi_data_[NCELL]` in `CellBlock`; `phi_rhs()` (conservative 1st-order upwind); `phi_compression_rhs()` (2-pass cell-centred scheme: ε·∇·(∇φ−φ(1−φ)n̂), n̂=∇φ/|∇φ|, eps_sq=1e-10/h²); SSP-RK3 phi integration via `phi_stage(α)` lambda; `use_acdi` + `acdi_ceps` config flags. Gate T12a/b/c (pure advection) ✅ and T13a/b/c (Cε=0.5 compression) ✅ in test_ns.cpp (20 t4 tests). Deferred: stiffened-gas EOS coupling (mixture γ, p∞), phi ghost fill for wall/open/AMR C/F faces. | `include/cell_block.hpp`, `include/operators.hpp`, `src/operators.cpp`, `src/block_tree.cpp`, `include/ns_solver.hpp`, `src/ns_solver.cpp`, `tests/test_ns.cpp` | partial |
