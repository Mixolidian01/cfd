...
|---|--------|------|-----------|--------|
| P13.1 | ✅ | **`template <Axis DIR>` flux refactor** — stages 1+2+3 ✅: `enum class Axis`, typed wrappers, all 15 int-axis calls in convective loop replaced, `kep_flux_t<DIR>` fully inlined with `constexpr` ternaries (dead branches elided at compile time), `weno5_face_t<DIR>` with `if constexpr` `idx_at`, compile-time `n_idx/t1_idx/t2_idx` constants. Runtime fallback `kep_flux/weno5_face(int axis)` retained for `undo_cf_face_flux/accumulate_fine_fluxes`. Stage 4 (viscous face-stress templating): ❌N/A — `viscous_rhs_impl` has no runtime axis dispatch; all 6 faces (x±,y±,z±) are computed explicitly in a single cell loop with fully named variables (dudx_xp, dvdy_yp…). Templating would add abstraction with zero dead-branch elimination benefit. | `include/operators.hpp`, `src/operators.cpp` | done |
| P13.2 | ✅ | **FDKEC mass flux upgrade** — Subbareddy & Candler (2009): mass = ½(ρL·u_nL + ρR·u_nR) replaces KG ρ_avg·u_n_avg in `kep_flux_t<DIR>` (inlined template) and runtime `kep_flux(int axis)`. Removes `rho_a` (unused after change). KE-consistent on uniform grids; KG momentum/enthalpy averages preserved. All gates t3+t4 (15+12 tests) pass. Note: viscous KEP split-form (Morinishi 2010) deferred. | `src/operators.cpp` lines 273-325 | done |
| P13.3 | ✅ | **Characteristic open BC with far-field pressure** — `SolverConfig::open_bc_p` (default 0 = zero-gradient); `BlockTree::set_open_bc_pressure()` module static; `open_char_ghost()` function: subsonic outflow → isentropic ρ_g=ρ·(p∞/p)^(1/γ), Riemann-invariant u_n_g = u_n_i + sign·2(c_g-c_i)/(γ-1); supersonic/inflow → zero-gradient. `open_x/y/z` lambdas replaced with `write_ghost(open_char_ghost(...))`. HLLC-ES then adds entropy dissipation when ghost≠interior. t2 (39/39) + t4 (12/12) pass. Note: SAT penalty formulation (Svärd-Gjesteland 2025) deferred; this is characteristic NSCBC-style. | `include/block_tree.hpp`, `src/block_tree.cpp`, `include/ns_solver.hpp`, `src/ns_solver.cpp` | done |
...
| P13.5 | ✅ | **SAT penalty at AMR C/F interfaces** — `tree_sat_penalty(tree, rhs, tau)` in `operators.cpp`: iterates fine leaves at C/F faces, computes σ*(Q_ghost−Q_interior)/h_f penalty on fine boundary cells, applies conservative coarse correction (σ*avg_patch/h_c) using `coarse_acc[NB/2][NB/2][NVAR]` stack buffer; `static_assert(NB/2<=4)`. `SolverConfig::sat_tau` (default 0.0 = disabled); called at each SSP-RK3 stage in `NSSolver::advance()` when `sat_tau>0 && max_leaf_level>0`. Gate: T11a mass error <1e-8 ✅, T11b SAT err ≤10× baseline ✅ (added to test_ns.cpp). Conservation proof: fine penalty = h_f²·σ·jump, coarse correction = h_c²/4·σ·avg_jump = h_f²·σ·avg_jump (since h_c=2h_f) → net zero in exact arithmetic. | `include/operators.hpp`, `src/operators.cpp`, `include/ns_solver.hpp`, `src/ns_solver.cpp`, `tests/test_ns.cpp` | done |
...
| # | Status | Item | Reference | Effort |
|---|--------|------|-----------|--------|
| P14.1 | ✅ | **ACDI compressible multiphase** — Full implementation: `phi_data_[NCELL]` in `CellBlock`; `phi_rhs()` (conservative 1st-order upwind); `phi_compression_rhs()` (2-pass Cε·h interface sharpening); SSP-RK3 phi integration via `phi_stage(α)`; phi ghost fill for wall/open/periodic/AMR C/F (5th-order Lagrange); prolongate/restrict extended to phi; Allaire 2002 mixture EOS (`mix_eos`, `eos_cons_to_prim_sg`); `set_sg_eos()` propagates γ_A/B, p∞_A/B to prim cache + HLLC-ES (corrected β=ρ/(2(p+p∞)) for SG enthalpy); `use_acdi`, `acdi_ceps`, `gamma_a/b`, `p_inf_a/b` config. All 26 t4 tests pass (T12–T15). | `include/cell_block.hpp`, `include/operators.hpp`, `src/operators.cpp`, `src/block_tree.cpp`, `include/ns_solver.hpp`, `src/ns_solver.cpp`, `tests/test_ns.cpp` | complete |
| P14.2 | ✅ | **Wall contact angle BC for phase-field** — Neumann wall BC: ∂φ/∂n=−cos(θ_w)/ε·g'(φ), g'(φ)=φ(1−φ)(1−2φ)/2; ghost fill: φ_ghost=φ_ref−dist·cos(θ_w)/ceps·g'(φ_ref), dist=cell count to wall; `contact_angle_wall` degrees in SolverConfig; `BlockTree::set_wall_contact_angle(cos,ceps)` static setter; backward-compatible (90°→Neumann). Gate T16a (neutral, phi unchanged) + T16b (wetting, wall cell changes). 28 t4 tests. | `include/ns_solver.hpp`, `include/block_tree.hpp`, `src/block_tree.cpp`, `src/ns_solver.cpp`, `tests/test_ns.cpp` | complete |
...
| P14.4 | ✅ | **GPU AMR subcycling** — GpuCfList (gpu_cf.cuh/cu) with k_cf_undo/accum/apply; _advance_amr() replaces CPU fallback; t24/t25/t26 pass | AMReX GPU subcycling (2025) | done |
| P14.5 | ✅ | **GPU ensemble UQ + Ensemble Kalman Filter** — EnsembleSolver (ensemble_solver.hpp/cpp): N NSSolver members, step_all, Welford compute_moments, stochastic EnKF with Gaspari-Cohn localization; t145 (E01–E04) pass | Finance-inspired UQ literature | done |
...
| P15.2 | ✅ | **MUSCL at block boundary faces** — threaded 6-bit `has_nbr` mask (one bit per `FaceDir`) from `tree_rhs`/`tree_rhs_typed` through `compute_rhs`→`convective_rhs_impl`→`accumulate_face`. Mask set only for same-level neighbor blocks (`neighbours[d] >= 0 && same level`). At boundary faces with `has_nbr=1`: van Albada MUSCL reconstruction (4-cell stencil fitting within NG=2 ghost region), then HLLC-ES. At domain-BC faces (`has_nbr=0`): previous PCM+HLLC-ES fallback unchanged. T08 rate remains 3.01 (single-block has_nbr=0 everywhere). `muscl_bnd_face<DIR>` inline template in `operators.hpp` shared by `accumulate_face` (convective_rhs.cpp) and `accumulate_face_typed` (operators.cpp). t3/t4/t6/t7 all pass. | Sweby (1984) TVD; van Albada (1982) | `include/schemes/operators.hpp`, `src/schemes/convective_rhs.cpp`, `src/schemes/operators.cpp` |
...
| 2 | P15.2 | ✅ | **MUSCL at block boundary faces** — `has_nbr` 6-bit mask threaded from `tree_rhs` to `accumulate_face`; van Albada MUSCL active only at same-level neighbor faces; T08 rate 3.01 preserved. | done |
...
| 6 | P14.4 | ✅ | **GPU AMR subcycling (Berger-Colella on GPU)** — `GpuCfList` (gpu_cf.cuh/cu): 3 kernels (k_cf_undo, k_cf_accum, k_cf_apply) mirror CPU undo_cf_face_flux + accumulate_cf_fine_fluxes + apply_flux_correction. `GpuGraphSolver::_advance_amr()` replaces the old CPU fallback for AMR trees. `ghost_filler.cpp` added to `_GPU_BASE` (pre-existing linker bug fixed). t24/t25/t26 all pass, including A4b (GPU+AMR mass conservation). Removed `max_leaf_level == 0` guard in ns_solver.cpp. | done |
| 7 | P14.5 | ✅ | **GPU ensemble UQ + EnKF** — `EnsembleSolver` (ensemble_solver.hpp/cpp): N NSSolver members, step_all (min-dt across members), Welford compute_moments, stochastic EnKF enkf_update with Gaspari-Cohn localization. t145 (E01–E04) all pass. | done |
| 8 | R5 | ✅ | **Instantiation matrix + branch-free dispatch** — 8 `extern template` suppressors added to `operators.hpp` (4×tree_rhs_typed + 4×compute_rhs_typed); `CpuRk3Integrator::select_scheme()` selects `tree_rhs_typed<>` specialisation once on first step, stored in `rhs_fn_` (branch-free hot loop). tr5 gate (S01–S03) all pass. 32/32 full gate. | done |
