...
| # | Status | Item | Reference | Effort |
|---|--------|------|-----------|--------|
| P14.1 | ⚠️ | **ACDI compressible multiphase** — Phase 1 (infrastructure + scalar advection): `phi_data_[NCELL]` added to `CellBlock` (flat SoA, separate from NVAR AoSoA, copy ctor/assign updated); `phi_rhs()` operator (conservative 1st-order upwind ∂φ/∂t = -∇·(φu)); phi ghost fill wired into `fill_ghosts_periodic` via `copy_cell` lambda extension; SSP-RK3 phi integration in `NSSolver::advance()` via `phi_stage(α)` lambda; `SolverConfig::use_acdi` flag; `NSSolver::init()` accepts optional `phi_ic` parameter. Gate T12a/b/c (in test_ns.cpp): phi conservation < 1e-8 ✅, no overshoot ✅, no undershoot ✅. Deferred: stiffened-gas EOS coupling (mixture γ, p∞), ACDI compression term (ε·∇·(∇φ − φ(1−φ)n̂)), phi ghost fill for wall/open/AMR C/F faces. | `include/cell_block.hpp`, `include/operators.hpp`, `src/operators.cpp`, `src/block_tree.cpp`, `include/ns_solver.hpp`, `src/ns_solver.cpp`, `tests/test_ns.cpp` | partial |
